...
### フォント最適化
- **システムフォント使用**: Webフォントを削除し、システムセリフフォントのみを使用
- **フォント遅延ロード問題の解決**: フォント読み込み遅延による視覚的な伸び縮みを完全に解決
- **パフォーマンス向上**: フォントファイルのダウンロードが不要でページ読み込みが高速
- **環境統一**: 開発・本番環境でフォント表示が統一
...
- **`src/layouts/Layout.astro`**: システムフォント設定
- **`src/components/ThemeProvider/index.tsx`**: システムフォント設定
- **`vercel.json`**: デプロイ設定
